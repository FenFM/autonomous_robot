...
short int ROBOT_BASE_X = 690;
short int ROBOT_BASE_Y = 90;
...
void DrawConsObject(struct World *world)
{
  ...
  Color(GREY_69);
  FillRectangle(500,WINDOW_W/2-41,36,36);
  object = CreateObject(world->ObjectType,965,1049,
...
{
  Color(GREY);
  FillRectangle(WINDOW_W/2+32,166,WINDOW_W/2-64,16);
  Color(BLUE);
  DrawText(WINDOW_W*3/4 - 4 * strlen(text),178,text);
}
...
void UndisplayComment(struct Context *context)
{
  ...
  switch(context->Info)
  {
    case INFO_ABOUT: DrawComment("About Khepera Simulator");
                     break;
    case INFO_USER0: DrawComment(context->UserInfo->Title[0]);
                     break;
    case INFO_USER1: DrawComment(context->UserInfo->Title[1]);
                     break;
    case INFO_USER2: DrawComment(context->UserInfo->Title[2]);
                     break;
    case INFO_USER3: DrawComment(context->UserInfo->Title[3]);
                     break;
  }
  XSync(display,FALSE);
}
...
{
  Color(GREY);
  FillRectangle(750,166,318,16);
  Color(BLUE);
  DrawText(750,178,text);
}
...
void DrawWorldSquare(struct World *world)
{
  ...
  DrawPlate(33,20,504,504,GREY_69,PLATE_DOWN);
...
void DrawWorld(struct Context *context)
{
  ...
  strcat(text,".world");
  Color(GREY);
  FillRectangle(2,2,566,16);
  Color(BLACK);
  DrawText(WINDOW_W/4-4*strlen(text),14,text);
  DrawWorldSquare(world);
  DrawPlate(499,WINDOW_W/2-42,38,38,GREY_69,PLATE_DOWN);
...
void DrawButton(struct Button *but)
{
  ...
  if (but->State == PLATE_UP) {x = but->X + 4; y = but->Y + 13; c = GREY;}
            else              {x = but->X + 5; y = but->Y + 14; c = GREY_69;}
...
{
  Color(GREY_69);
  FillRectangle(WINDOW_W/2+234,44,180,53);
  Color(BLACK);
  if (robot->State & REAL_ROBOT_FLAG)
   DrawText(WINDOW_W/2+236,54,"real Khepera");
  else DrawText(WINDOW_W/2+236,54,"simulated Khepera");
  if (robot->State & DISTANCE_SENSOR_FLAG)
   DrawText(WINDOW_W/2+236,74,"distance sensor values");
  else if (robot->State & LIGHT_SENSOR_FLAG)
   DrawText(WINDOW_W/2+236,74,"light sensor values");
  else DrawText(WINDOW_W/2+236,74,"sensor activities");
  if (robot->State & MOTOR_VALUES_FLAG)
   DrawText(WINDOW_W/2+236,94,"motor values");
  else DrawText(WINDOW_W/2+236,94,"motor activities");
}
...
void DrawRobotLegend(struct Robot *robot)
{
  ...
  Color(BLACK);
  DrawText(WINDOW_W/2+218,140,"Activity");
  DrawText(WINDOW_W/2+296,126,"Low");
  DrawText(WINDOW_W/2+434,126,"High");
  for(i=0;i<9;i++)
   DrawPlate(WINDOW_W/2+296+i*20,131,10,10,BLUE+i,PLATE_UP);
}
...
void DrawRobot(struct Context *context)
{
  ...
  robot = context->Robot;
  Color(GREY);
  FillRectangle(WINDOW_W/2+2,2,WINDOW_W/2-4,16);
  Color(BLACK);
  strcpy(title,robot->Name);
  strcat(title,".robot");
  DrawText(WINDOW_W*3/4-4*strlen(title),14,title);
  DrawPlate(WINDOW_W/2+32,20,504,140,GREY_69,PLATE_DOWN);
  DrawRobotBase(robot);
...
void DrawWindow(struct Context *context)
{
  ...
  XSync(display,TRUE);
  robot = context->Robot;
  DrawPlate(0,0,WINDOW_W/2,WINDOW_W/2,GREY,PLATE_UP);
  DrawWorld(context);
  DrawLittleRobot(robot,robot);
  DrawPlate(WINDOW_W/2,0,WINDOW_W/2,WINDOW_W/2,GREY,PLATE_UP);
  DrawRobot(context);
  DrawInfo(context);
  but = context->Buttons;;
  while(but)
